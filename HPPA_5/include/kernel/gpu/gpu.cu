...
void sort(const unsigned int** ls_buffer_ptr, unsigned int** ls_sorted_buffer_ptr)
{
	...
	for (int counter = 0; counter < ITERATIONS_PER_THREAD; counter++) {
		const unsigned int OFFSET = blockDim.x * counter;

		for (int counter_1 = 0; counter_1 < 3; counter_1++) {
			ls_column[counter_1] = ls_buffer_ptr[counter_1][threadIdx.x + OFFSET];
		}
		...
		for (int counter_1 = 0; counter_1 < 3; counter_1++) {
			ls_sorted_buffer_ptr[counter_1][threadIdx.x + OFFSET] = ls_sorted_column[counter_1];
		}
	}
